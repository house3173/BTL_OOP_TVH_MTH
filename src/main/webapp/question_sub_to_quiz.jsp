...
<div class="container" style="border: 1.5px solid #ebebeb; border-radius:5px; height:auto; margin-top:20px; padding: 20px 25px;">
	...
	<form method="get" id="form_select_category">
        ...
        <% for (int i=0; i< categoryList.size(); i++) { 
			...
			category cateselectbyname = ctd.selectByParent(nameCate);
			ArrayList<String> subname = listcategory.SubCategory(cateselectbyname);
			int countsubquestion = 0;
			for (int j = 0; j<subname.size(); j++) {
				countsubquestion += quedao.selectAllByNameCategory(subname.get(j)).size();
			}
			String countSubQuestion=(countsubquestion>0)?"("+String.valueOf(countsubquestion)+")":"";
        ...
        %>
            ...
            &nbsp; <%} %>
            <%=nameCate%>&nbsp;&nbsp;<%=countSubQuestion %></option>
        <% } %>
		</select>
		...
		<div class="form-check">
			  <input class="form-check-input" type="checkbox" checked value="" id="question_subcategory" name="question_subcategory"  onclick="setFormAction()">
...
<form action="question_to_quiz" method="post" id = "category_selected" style="display:block;">
	 	 <div id="selectAll" style="margin-left:6px; margin-bottom:5px;" >
		 	 <input  type="checkbox" id="selectAllQuestion" name="selectAllQuestion"  >
	...
	<%
			...
			ArrayList<String> subname2 = listcategory.SubCategory(cateselect2);
			int count2 = 0;
	%>
	
	 <div class="listquestion"  style="display:block;">
		  <%
		  for (int h = 0; h<subname2.size(); h++) {
			  String subName  = subname2.get(h);
			  ArrayList<question> listsubquestion = new ArrayList<question>();			
			  listsubquestion = quesdao2.selectAllByNameCategory(subName);
		  
			  for (int k =0; k < listsubquestion.size(); k++) {
				  count2++;
			 	 question que = listsubquestion.get(k);
				 ...
				 String color = (count2%2==1)?"#ebebeb":"white"; 
...
<!--  END -->
	 ...
	 <div style="margin-left:25px; margin-bottom:15px; display:none;" id="OnlyTextQuestion"><b>Question</b></div>
	<%
		ArrayList<category> listcate = new ArrayList<category>();
		...
		for (int j = 0; j < listcate.size(); j++) {
			...
			category cateselect = catedao.selectByParent(cateselected.getNameCategory());
			ArrayList<String> subname = listcategory.SubCategory(cateselect);
			int count = 0;
	%>
	
	 <div class="listquestion" id="<%=idcateselected %>"  style="display:none;">
		  <%
		  for (int h = 0; h<subname.size(); h++) {
			  String subName  = subname.get(h);
			  ArrayList<question> listsubquestion = new ArrayList<question>();			
			  listsubquestion = quesdao.selectAllByNameCategory(subName);
			  ...
			  for (int k =0; k < listsubquestion.size(); k++) {
			 	 ...
			 	 String strquestion = que.getTextQuestion()+": 1."+que.getChoice1().getTextChoice()
					  +" 2."+que.getChoice2().getTextChoice()+" 3."+que.getChoice3().getTextChoice()
					  +" 4."+que.getChoice4().getTextChoice()+" 5."+que.getChoice5().getTextChoice();
				 String color = (count%2==1)?"#ebebeb":"white"; 
		  		 String namequestion = que.getNameQuestion();
...
<script>
		...
		function changeSelect() {
			...
			document.getElementById("OnlyTextQuestion").style.display="block";
		}
		...
		var checkAllCheckbox = document.getElementById("selectAllQuestion");
